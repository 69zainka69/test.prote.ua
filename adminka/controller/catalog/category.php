...
class ControllerCatalogCategory extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST')){
            if (!$this->request->post['keyword']) {
                $this->request->post['keyword'] = translit($this->request->post['category_description'][1]['name']);
            }
	...
	protected function getForm() {
        ...
        $data['entry_informal'] = $this->language->get('entry_informal');
        ...
        $data['help_informal'] = $this->language->get('help_informal');
		...
		if (isset($this->request->post['children'])) {
			$data['childrens'] = $this->request->post['children'];
		} elseif (isset($this->request->get['category_id'])) {
			$data['childrens'] = $this->model_catalog_category->getCategoryChildrens($this->request->get['category_id']);
		} else {
			$data['childrens'] = array();
		}

		// Categories
		$categoryRecommendation = $this->model_catalog_category->getRecommendationCategories($this->request->get['category_id']);
		$data['categoriesRecommendation'] = $categoryRecommendation;
		...
		/////////////////////////
		// сопоставление категории с аксаптой
		$res = $this->db->query("SELECT * FROM axapta_GroupSite2List WHERE LanguageId=1 ORDER BY GroupSite2Id ");
		$data['GroupSite2List']['brain']  =array(
			'GroupSite2Id' =>'brain',
			'GroupSite2Name' =>' Категория Брейна'
		);

		$data['GroupSite2List'] = $data['GroupSite2List']+ $res->rows;

		if (isset($this->request->post['GroupSite2_id'])) {
			$data['GroupSite2Id'] = $this->request->post['GroupSite2_id'];
		} elseif (!empty($category_info)) {
			$data['GroupSite2Id'] = $category_info['GroupSite2Id'];
		} else {
			$data['GroupSite2Id'] = 0;
		}
        ...
        if (isset($this->request->post['questions'])) {
            ...
            $data['questions'] = $this->model_catalog_category->getCategoryQuestions($this->request->get['category_id']);
        } else {
		...
		if (isset($this->request->post['top'])) {
   		...
   		if (isset($this->request->post['informal'])) {
			$data['informal'] = $this->request->post['informal'];
		} elseif (!empty($category_info)) {
			$data['informal'] = $category_info['informal'];
		} else {
			$data['informal'] = 0;
		}
