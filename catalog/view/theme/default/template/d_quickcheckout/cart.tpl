<!-- Ajax Quick Checkout v6.0.0 Dreamvention.com d_quickcheckout/cart.tpl --> 
<style>
 #payment_address_form input[type="email"], #payment_address_form input[type="password"], #login_password {line-height:30px;padding:0 10px;width:100%;font-size:11.5px;color:#999;margin-bottom:10px;} 
 #payment_address_customer_group_id_input .radio {margin-bottom: 4px;}
</style>
<div id="cart_view" class="qc-step" data-col="3" data-row="3"></div>
<script type="text/html" id="cart_template">
	<div class="panel panel-default <%= model.config.display ? '' : 'hidden1' %>">
		...
		<div class="qc-checkout-product panel-body" >
			...
			<div class="form-horizontal qc-totals">
				...
				<!-- Delete all totals -->
        <!-- <% total=_.last(model.totals) %> -->
        <% _.each(model.totals, function(total) { %>
        <% if(total.code=='sub_total'){ %>
        <% } else { %>
				<div class="row">
					<label class="col-sm-9 col-xs-6 control-label" ><%= total.title %></label>
					<div id="sum_<%= total.code%>" class="col-sm-3 col-xs-6 form-control-static text-right"><%= total.text %></div>
				</div>
        <% } %>
        <% }) %>
				<!-- ------------------- -->
...
prods = products['products'];html='';

$(prods).each(function( index ) {
    html+='{"id":'+this['model']+',';
    html+='"name":'+this['name']+',';
    html+='"list_position":'+index+',';
    html+='"quantity":'+this['quantity']+',';
    html+='"price":'+this['price_float']+'}';
    if(prods.length-1==index){html+=',';}
});
//console.log(html);
gtag('event', 'begin_checkout', {"items": [ html ]});
});
