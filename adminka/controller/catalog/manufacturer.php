...
class ControllerCatalogManufacturer extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if(isset($this->request->post['filter_ids'])){
				$this->request->post['filter_ids'] = implode(',',$this->request->post['filter_ids']);
			}
	...
	protected function getForm() {
		...
		/*if (isset($this->request->post['filter_id'])) {
			...
			$filter = $this->model_catalog_filter->getFilter($this->request->post['filter_id']);
			//vdump($filter);
			if($filter){ $data['filter_name']=$filter['name'];}

		} elseif (!empty($manufacturer_info)) {
			$data['filter_id'] = $manufacturer_info['filter_id'];

			$this->load->model('catalog/filter');
			$filter = $this->model_catalog_filter->getFilter($manufacturer_info['filter_id']);
			if($filter){ $data['filter_name']=$filter['name'];}
		...
		if (isset($this->request->post['filter_ids'])) {
			$data['filter_ids'] = $this->request->post['filter_ids'];

			$this->load->model('catalog/filter');
			foreach ($this->request->post['filter_ids'] as $key => $value) {
				$manufacturer = $this->model_catalog_filter->getFilter($value);
				$manufacturer['name'] ='[filter_group_id='.$manufacturer['filter_group_id'] .', filter_id='.$manufacturer['filter_id'] .'] - '.$manufacturer['name'];
				$filters[] = $manufacturer;
			}
			$data['filter_ids'] = $filters;
			//vdump($filters);
			//if($filter){ $data['filter_name']=$filter['name'];}

		} elseif (!empty($manufacturer_info)) {
			$data['filter_ids'] = explode(',',$manufacturer_info['filter_ids']);
	...
	public function autocompleteFilter() {
		...
		if (isset($this->request->get['filter_name'])) {
			//$this->load->model('module/manufacturer');
			$this->load->model('catalog/manufacturer');
			...
			$manufacturers = $this->model_catalog_manufacturer->getFilterManufacturers($filter_data);

			foreach ($manufacturers as $manufacturer) {
				$json[] = array(
					'filter_id' => $manufacturer['filter_id'],
					'name'      => strip_tags(html_entity_decode('[filter_group_id='.$manufacturer['filter_group_id'] .', filter_id='.$manufacturer['filter_id'] .'] - '.$manufacturer['name'], ENT_QUOTES, 'UTF-8'))
				);
			}
		}
