...
class ControllerDQuickcheckoutPaymentAddress extends Controller {
    ...
    public function index($config){
        ...
        if(isset($this->session->data['payment_address']['customer_group_id']) && $this->session->data['payment_address']['customer_group_id']=='4'){
            $data['customer_group_id'] = 4;
        }
        $json['customer_group_id'] =  $data['customer_group_id'];
        ...
        if($this->customer->isLogged()){
            ...
            $data['fio'] = $this->customer->getFirstname().' '.$this->customer->getLastname();
            $json['addresses'] = $this->model_d_quickcheckout_address->getAddresses();
            $json['payment_address']['firstname'] = $this->customer->getFirstname();
            $json['payment_address']['lastname'] = $this->customer->getLastname();
            $json['payment_address']['email'] = $this->customer->getEmail();
            $json['payment_address']['telephone'] = $this->customer->getTelephone();
            $json['payment_address']['custom_field'] = [ 'account' => [1 => '', 2 => '', 3 => '']];
            $json['payment_address']['custom_field.account.2'] = '';
            $json['payment_address']['custom_field.account.1'] = '';
            $json['payment_address']['custom_field.account.3'] = '';
            $customer_group_id = $customer['customer_group_id'];
            $data['customer_group_id'] = $customer_group_id;
            $this->session->data['payment_address']['customer_group_id'] = $customer_group_id;
            $json['payment_address']['customer_group_id'] = $customer_group_id;
            $json['account'] = 'logged';
            $custom_field = (array)json_decode($customer['custom_field']);
            $json['payment_address']['fax'] = $custom_field[1];
            $json['payment_address']['company'] = $custom_field[2];
		}
    ...
    public function prepare($json){
        ...
        if(isset($this->request->post['payment_address'])){
            ...
            if($this->customer->isLogged()){
                ...
                if(!empty($this->request->post['payment_address']['company'])) {
                    $this->request->post['payment_address']['custom_field']['account'][1] = $this->request->post['payment_address']['fax'];
                    $this->request->post['payment_address']['custom_field']['account'][2] = $this->request->post['payment_address']['company'];
                }
          ...
          //   print_r(  $this->session->data['payment_address'] );
            ...
            if($this->customer->isLogged()){
                $customer = $this->model_account_customer->getCustomer($this->customer->getId());
                $custom_field = (array)json_decode($customer['custom_field']);
                if(!empty($this->request->post['payment_address']['company']) && !empty($this->request->post['payment_address']['fax'])) {
                    $this->request->post['payment_address']['custom_field'] = [];
                    $this->request->post['payment_address']['custom_field'][1] = $this->request->post['payment_address']['fax'];
                    $this->request->post['payment_address']['custom_field'][2] = $this->request->post['payment_address']['company'];
                    $this->request->post['payment_address']['custom_field'][3] = $custom_field[3];
                }
        ...
        //session
        if($this->customer->isLogged()){
            $telephone = $this->request->post['payment_address']['telephone'];
            $this->model_account_customer->editCustomer($this->request->post['payment_address']);
           /* var_dump($telephone);
            die();*/
