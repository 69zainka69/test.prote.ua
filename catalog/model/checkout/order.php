...
class ModelCheckoutOrder extends Model {
	...
	public function editOrder($order_id, $data) {
		...
		$this->addOrderHistory($order_id, 3);
	...
	public function getOrder($order_id) {
		...
		if ($order_query->num_rows) {
			...
			return array(
				...
				'no_call'                     => $order_query->row['no_call'],
                ...
                'is_complex'              => $order_query->row['is_complex']
			);
		} else {
			return false;
		}
	...
	public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false, $override = false) {
		...
		if ($order_info) {
			...
			if (!in_array($order_info['order_status_id'], array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status'))) && in_array($order_status_id, array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status')))) {
                ...
                $order_product_query = $this->db->query("SELECT op.*, p.mpn FROM " . DB_PREFIX . "order_product op LEFT JOIN  " . DB_PREFIX . "product p using(product_id) WHERE order_id = '" . (int)$order_id . "'");
			...
			if (!$order_info['order_status_id'] && $order_status_id) {
				...
				$data['text_new_company'] = $language->get('text_new_company');
				$data['text_new_company_name'] = $language->get('text_new_company_name');
				$data['text_new_no_clall'] = $language->get('text_new_no_clall');
				$data['text_new_no_call_tooltip'] = $language->get('text_new_no_call_tooltip');

				$data['shipping_company'] = $order_info['shipping_company'];
				$data['company_name'] = $order_info['fax'];
				$data['no_call'] = $order_info['no_call'];
				...
				// Products
				$data['products'] = array();
                //$this->log->write(print_r($order_product_query->rows,1));
				foreach ($order_product_query->rows as $product) {
					...
					$data['products'][] = array(
						...
						'pricefloat'    => round($product['price'],2),
                ...
                if($order_info['email']){
    				...
    				$mail->setTo($order_info['email'] ? $order_info['email'] : $this->config->get('config_email'));
				...
				if ($this->config->get('config_order_mail')) {
					...
					//$mail->setTo('nata@vm.ua');
					$mail->setTo('internet_sales@vm.ua');
					$mail->setFrom($this->config->get('config_email'));
					$mail->setSender(html_entity_decode($order_info['store_name'], ENT_QUOTES, 'UTF-8'));
					$mail->setSubject(html_entity_decode($subject, ENT_QUOTES, 'UTF-8'));
					$mail->setHtml($html);
				...
				$data['shipping_address'] = strip_tags(str_replace(array('<br/>','<br />'),' ',$data['shipping_address']));

				$customer['name'] = $order_info['payment_firstname'].' '.$order_info['payment_lastname'];
				$customer['email'] = $data['email'];
				$customer['phone'] = $data['telephone'];
				...
				$shipping_methods = array(
					'pickup.pickup' =>'1', // самовывоз
					'free.free' 	=>'6', // Ночной Экспресс
					'flat.flat' 	=>'3', // доставка по киеву
					'novaposhta.novaposhta' =>'4', // Новая почта
					'justin.justin' =>'10', // justin
					'meest.meest' =>'11', // meest
					'intime.intime' =>'8', // интайи
					'ukrpost.ukrpost' =>'9' // укрпочта
				);
				//$this->log->write($order_info);
				if(isset($shipping_methods[$order_info['shipping_code']])){
					$DeliveryMethod = $shipping_methods[$order_info['shipping_code']];
				} else {
					$DeliveryMethod = 1;
				}
				if($order_info['shipping_code']=='ukrpost.ukrpost' || $order_info['shipping_code']=='flat.flat'){
					$data['shipping_address'] .=' '.$order_info['shipping_method'];
				}
				// кесли способ оплаты портмоне, очищаем коммент т.к. там длинная ссылка на чек, аксапта ее не принимает
				if(isset($order_info['payment_code']) && $order_info['payment_code']=='portmone' || isset($data['payment_method']) && $data['payment_method']=='Portmone'){
					$data['comment'] ='';
				} elseif(isset($order_info['payment_code']) && ($order_info['payment_code']=='bank_transfer2' || $order_info['payment_code']=='bank_transfer')) {

					$data['comment'] = $order_info['comment'];
				}
if($DeliveryMethod==10){
	$string = $data['shipping_address'];
	$strin = explode("№", $string);
	$stri = explode(",", $strin[1]);
	$str = explode(" ", $stri[0]);
	$DeliveryRegion = $str[1];


$dev_city = explode(",", $string);
$dev_citys = explode(" ", $dev_city[0]);
$DeliveryCity=$dev_citys[2]; 
$dev_number = explode("№", $string);
$dev_number = explode(" ", $dev_number[1]);
$strWithoutChars = preg_replace('/[^0-9]/', '', $dev_number[0]);
$DeliveryNumber=$strWithoutChars;
$street = $order_info['shipping_address_2'];
$str = explode(",", $street);
$st="На склад Justin:".$customer['name']." ".$str[1].", ".$str[2].", ".$str[3].", ".$str[4];
	$order = array();
	$order['CustPaymMode'] = $data['payment_method'];
	$order['DoNotCallBack'] = $data['no_call'];
	...
	$order['DeliveryStreet'] = "$st";
	$order['DeliveryHouse'] = "";
	$order['DeliveryFlat'] ='';
	$order['OrderLines'] =$OrderLines;
	$order['Comment'] =$data['comment'];
	$order['isComplex'] = $order_info['is_complex'];
	...
	$this->model_checkout_axapta->addOrder($customer,$order);
		...
		if($DeliveryMethod==11){
			$string = $data['shipping_address'];
			$strin = explode("№", $string);
			$stri = explode(",", $strin[1]);
			$str = explode(" ", $stri[0]);
			$DeliveryRegion = mb_strtolower($str[1]);
			$DeliveryRegion = ucfirst($DeliveryRegion);
		
		
		$dev_city = explode(",", $string);
		$dev_citys = explode(",", $dev_city[2]);
		$DeliveryCity=$dev_citys[0]; 
		$dev_number = explode("№", $string);
		$dev_number = explode(" ", $dev_number[1]);
		$strWithoutChars = preg_replace('/[^0-9]/', '', $dev_number[0]);
		$DeliveryNumber=$strWithoutChars;
		$street = $order_info['shipping_address_2'];
		$str = explode(",", $street);
		$st="На почтомат meest:".$customer['name']." ".$str[1].", ".$str[2].", ".$str[3]." ".$str[4];
			$order = array();
			$order['CustPaymMode'] = $data['payment_method'];
			$order['DoNotCallBack'] = $data['no_call'];
			$order['DeliveryMethod'] = $DeliveryMethod;//$data['payment_method'];
			...
			$this->model_checkout_axapta->addOrder($customer,$order);
			    ...
			    $order['DeliveryStreet'] = $data['shipping_address'];
			    $order['DeliveryHouse'] = '';
			    $order['DeliveryFlat'] ='';
			    $order['OrderLines'] =$OrderLines;
			    $order['Comment'] =$data['comment'];
	...
	/*public function shopCustInsert($name,$email,$phone,$discount='',$address='',$regtype='prote.ua') {
