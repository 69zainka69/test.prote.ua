...
if (typeof BrainyFilter === 'undefined') {
    var BrainyFilter = {
        ...
        init: function() {
            ...
            // Manual disable ajax for seo address functionality
            this.ajaxEnabled =  false;
            // -------------------------------------------------
            this.redirectTo = this.redirectTo.replace(/\&amp;/g, '&');
...
//                BrainyFilter.sendRequest();
        ...
        reset: function() {
            ...
            jQuery(this.filterFormId + ' input:checked').prop('checked',false);
            ...
            if (BrainyFilter.ajaxEnabled && !BrainyFilter.redirectTo) {
                ...
                this.ajaxHandler = jQuery.ajax({
                    ...
                    success: function(res) {
                        ...
                        // --
                         var newUrlQuery = BrainyFilter.prepareFilterData(true, curRoute);
                         console.log('n'+newUrlQuery);
                         console.log(curRoute);
                        // --

                        var preroute=jQuery(BrainyFilter.filterFormId).find('input[name="route"]').val()
                        var prepath =jQuery(BrainyFilter.filterFormId).find('input[name="path"]').val()
                        var langcode = window.location.pathname.substr(1,2)


                        // BrainyFilter.redirectTo=(langcode=='ua' ? 'ua/' : '')+'index.php?route='+preroute+'&path='+prepath // +'&lang=uk'
                        // newUrlBase=(langcode=='ua' ? 'ua/' : '')+'index.php?route='+preroute+'&path='+prepath // +'&lang=uk'
                        newUrlBase='index.php?route='+preroute+'&path='+prepath // +'&lang=uk'
                        newUrl = newUrlBase + (newUrlBase.match(/\?/) ? '&' : '?') + BrainyFilter.serializeFilterForm();
                        //console.log(newUrl);
                        jQuery.ajax({
                            url: '/index.php?route=common/seo_pro/arewrite/',
                            data: { url: newUrl },
                            dataType:'json',
                            type: 'get',
                            error: function(err) {
                            ...
                            success: function(data) {
                                var langcode = window.location.pathname.substr(1,2);
                                if (langcode=='ua') {
                                    data=data.replace('prote.com.ua', 'prote.com.ua/ua');
                                }
                                data=data.replace(/\&amp;/g, '&');
                                console.log('data='+data);
                                newUrl = data;

                                window.history.pushState({"html": document.getElementById("ajcontainer").innerHTML, "pageTitle": jQuery('title').text()}, "", newUrl);

                                window.onpopstate = function(e){
                                    console.log('e-object:',e);
                                    if(e.state.html){
                                        document.getElementById("ajcontainer").innerHTML = e.state.html;
                                        document.title = e.state.pageTitle;
                                    }
                                };
                                newTitle=$("#brainyfilter-product-header").data('h1'); console.log(newTitle);
                                $('h1').html(newTitle);
                                $('#lastbreadcrumb').html(newTitle);
                                $(document).attr("title", newTitle);

                            },
                        });

//                        ('/index.php?route=common/seo_pro/arewrite/&url=' + encodeURIComponent(newUrl), function(data) {
//                            newUrl2=data
//                            // alert(data)
//                        });
                ...
                //console.log (preroute+prepath);
                BrainyFilter.redirectTo=(langcode=='ua' ? 'ua/' : '')+'index.php?route='+preroute+'&path='+prepath; // +'&lang=uk'

                //console.log(BrainyFilter.redirectTo);
                ...
                jQuery.ajax({
                    url: '/index.php?route=common/seo_pro/arewrite/'+(langcode=='ua' ? '&lang=ua' : ''),
                    data: { url: newUrl},
                    dataType:'json',
                    type: 'get',
                    success: function(data) {
                        if(data){
                            data = data.replace('\\','');
                            window.location = data; //data.replace('&amp;','&');
                            return false;
                        } else {
    ...
    //        var searchFilters = {};
        ...
        changeTotalNumbers: function(data) {
            ...
            /* Запрет показывать строчки с нулевыми значениями ибо нефиг...
        ...
        shrinkBlock: function(block, items, disableAnim) {
            ...
            if (disableAnim) {
                ...
                jQuery(block).closest('.bf-attr-block').find('.bf-arrow').toggleClass('close');
            }
            if (!items) {
                //jQuery(block).closest('.bf-attr-block').find('.bf-arrow').css('background-position', '50% -128px');
                //jQuery(block).closest('.bf-attr-block').find('.bf-arrow').toggleClass('close');
                jQuery(block).closest('.bf-attr-block').find('.bf-arrow').removeClass('open');
                jQuery(block).closest('.bf-attr-block').find('.bf-arrow').addClass('close');
        ...
        expandBlock: function(block, disableAnim) {
            ...
            jQuery(block).closest('.bf-attr-block').find('.bf-arrow').removeClass('close');
            jQuery(block).closest('.bf-attr-block').find('.bf-arrow').addClass('open');
        },
