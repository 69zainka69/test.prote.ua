...
///// документация http://api.brain.com.ua/help
class Apibrain {
    
  private $login = 'nata@vm.ua';
  private $password = 'nata@vm.ua';
  //  private $login = 'ustx.service@gmail.com';
  //  private $password = 'ghedtn6262';
    private $token;

  private $url = 'http://api.brain.com.ua/';
  ...
  private $registry;
    ...
    $result=json_decode($this->callpost('auth', $data),true);
    
    if (isset($result['error_code'])&&$result['error_code']>0) die('Ошибка при авторизации: ' . $result['error_message']);
    ...
    function __destruct() {
       $this->callpost('logout', array('SID'=>$this->token));
    }

    

    public function callpost($method, $data = array()) {


    // echo $this->url . $method . ($this->token ? '/'.$this->token : '');
        $datastr = http_build_query($data);
     // echo $this->url . $method . ($this->token ? '/'.$this->token : ''.$datastr);

        $defaults = array(
            CURLOPT_POST            => 1,
            CURLOPT_HEADER          => 0,
            CURLOPT_URL             => $this->url . $method . ($this->token ? '/'.$this->token : ''),
    ...
    public function call($method, $data = array()) {
        ...
        $defaults = array(
            ...
            CURLOPT_URL             => $this->url . $method . ($this->token ? '/'.$this->token : '') .($datastr ? '?' . $datastr : ''),
            // CURLOPT_USERAGENT       => 'OpenBay Pro for Amazon/Opencart',
            // CURLOPT_FRESH_CONNECT   => 1,
            CURLOPT_RETURNTRANSFER  => 1,
            // CURLOPT_FORBID_REUSE    => 1,
