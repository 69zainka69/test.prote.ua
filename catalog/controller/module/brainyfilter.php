...
class ControllerModuleBrainyFilter extends Controller {
    ...
    {   //   print_r($moduleSettings); echo '--';
        ...
        $settings = $this->_getSettings($moduleSettings['bf_layout_id']);  //  var_dump($settings);
...
$lng =  $this->language->load('product/product');
        ...
        if (preg_match('/(iPhone|iPod|iPad|Android)/', $_SERVER['HTTP_USER_AGENT'])) {
            if ($isMijoShop) {
                ...
                //$this->document->addScript('catalog/view/javascript/jquery.ui.touch-punch.min.js','footer','async');
            }
        }
        if ($isMijoShop) {
            MijoShop::get('base')->addHeader(JPATH_MIJOSHOP_OC . '/catalog/view/javascript/brainyfilter.js', false);
        } else {
            $this->document->addScript('catalog/view/javascript/brainyfilter.js?5','header','async');
        }
        ...
        $this->_data['text_selected_filters']    = $this->language->get('text_selected_filters');
        $this->_data['button_clear']    = $this->language->get('button_clear');
        $this->_data['filter_close']    = $this->language->get('filter_close');
        ...
        $filters = array();
        $secSettings = $settings['behaviour']['sections']; // var_dump($secSettings);
        ...
        if ($secSettings['filter']['enabled']) {
            ...
            if (count($arr)) {
                $filters[] = array(
                    ...
                    'array' => $this->model_catalog_product->getBrainyFiltersAttributesColorCyrillic($arr),
                     // 'array' => $arr,
...
//            }
        ...
        usort($filters, array(__CLASS__, '_sortProperties')); // print_r($filters);
...
//   var_dump($data);
    ...
    {   // echo '>'.$layoutId.':';
        $bfSettings = array();
        // var_dump($this->config->get('brainyfilter_layout_basic'));
