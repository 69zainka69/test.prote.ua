...
class ModelTotalCoupon extends Model {
	public function getCoupon($code) {
		...
		//$coupon_query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "coupon` WHERE code LIKE '%" . $this->db->escape($code) . "%' AND ((date_start = '0000-00-00' OR date_start < NOW()) AND (date_end = '0000-00-00' OR date_end > NOW())) AND status = '1'");
		...
		$coupon_query->row = array();
		$coupon_query->num_rows = 0;
		foreach ($coupon_query->rows as $key => $row) {
			$codes = explode(',',$row['code']);
			if(in_array($code,$codes)){
				$coupon_query->row = $row;
				$coupon_query->num_rows =1;
				break;
			}
