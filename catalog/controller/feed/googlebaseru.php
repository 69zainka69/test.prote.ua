...
class ControllerFeedGoogleBaseru extends Controller {
	public function index() {
		if ($this->config->get('google_base_status')) {
			...
			$output .= '<channel>';
			$output .= '<title>Prote - канцтовары, картриджи, бумага, чернила. Все, что нужно офиса.</title>';
            ...
            if (!in_array($product['product_id'], $product_data)) {
                ...
                foreach ($attribute_groups as $attribute_group) {
                    foreach ($attribute_group['attribute'] as $attribute) {
                        ...
                        if($attribute['attribute_id']==13 && ($attribute['text']=='Сумісний'||$attribute['text']=='Совместимый')){
                        /*if($attribute['attribute_id']==13){*/

                            if($attribute['text']=='Совместимый'){
                                $analog='Совместимый';
                ...
                foreach ($categories as $category) {
                    ...
                    if ($path) {
                        ...
                        foreach (explode('_', $path) as $path_id) {
                            $category_info = $this->model_catalog_category->getCategoryeru($path_id);
                        ...
                        $output .= '<g:product_type>Главная > ' . $string . '</g:product_type>';
                    }
                }
