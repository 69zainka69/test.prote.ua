	...
	public function callback() {
  	...
  	public function confirm() {
		...
		if ($this->session->data['payment_method']['code'] == 'privat24') {
			...
			$comment .= $this->config->get('privat24' . $this->config->get('config_language_id')) . "\n\n";
			$comment .= $this->language->get('text_payment');

			$this->model_checkout_order->addOrderHistory($this->session->data['order_id'], $this->config->get('privat24_order_status_id'), $comment, true);
		}
	}
