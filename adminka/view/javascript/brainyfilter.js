    ...
    fillLayoutSelectbox : function() {
        ...
        for (var m in this.settings) {
            ...
            if (m !== 'basic') {
                ...
                catlistfull = this.settings[id].categories;
                catlist='';
                for (var catitem in catlistfull) {
                   
                   if (catlistfull[catitem]=='1') {
                      catlist = catlist + ' ' + catitem; // this.categoryLayouts[catitem];
                   }
                }
                var layoutOpt = jQuery('<option value="'+id+'">' + layoutName + ' / ' +catlist.trim() +'</option>');
...
//                console.log(name + ' - not found');
    ...
    refreshDB : function() {
        ...
        if (!btn.hasClass('wait')) {
            btn.addClass('wait');
            var url = BrainyFilterAdm.refreshActionUrl.replace('&amp;', '&'); console.log(url);
