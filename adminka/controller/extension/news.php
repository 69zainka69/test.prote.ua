...
class ControllerExtensionNews extends Controller {
	...
	protected function form() {
		...
		$data['text_addrelated'] = $this->language->get('text_addrelated');
		...
		if (isset($this->request->get['news_id'])) { 
			$news = $this->model_extension_news->getNews($this->request->get['news_id']);
                        // Related products                         
                        $related = $this->model_extension_news->getRelated($this->request->get['news_id']);
                        if ($related) {
                            
                            $this->load->model('catalog/product');
                            
                            foreach ($related as $item) { 
                                $products[]=$this->model_catalog_product->getProduct($item['product_id']);
                            }
...
//		}
		...
		$this->load->model('tool/image');
                ...
                foreach ($data['news'] as $key=>$val) {
                    $data['news'][$key]['thumb']= $this->model_tool_image->resize($data['news'][$key]['image'] ? $data['news'][$key]['image'] : 'no_image.png', 100, 100);
                }		
		...
		///// 2019.11.08 - gdemon - hotline /////////////////
		if (isset($this->request->post['date_start'])) {
			$data['date_start'] = $this->request->post['date_start'];
		} elseif (!empty($news)) {
			$data['date_start'] = $news['date_start'];
			$data['date_start'] = str_replace('0000-00-00','',$data['date_start']);
		} else {
			$data['date_start'] = '';
		}
		...
		if (isset($this->request->post['date_end'])) {
			...
			$data['date_end'] = $news['date_end'];
			$data['date_end'] = str_replace('0000-00-00','',$data['date_end']);
		} else {
			$data['date_end'] = '';
		}
		...
		if (isset($this->request->post['gift'])) {
			$data['gift'] = $this->request->post['gift'];
		} elseif (!empty($news)) {
			$data['gift'] = $news['gift'];
		} else {
			$data['gift'] = '';
    ...
    public function addRelated() {
            ...
            foreach ($product_ids as $key => $product_id) {
	            ...
	            // Поиск продукта 
	            if ($ret=$this->model_extension_news->getProductBy($product_id)) { 
	                $this->model_extension_news->addRelatedProduct($news_id, $ret['product_id']);
	            }
	        }
            // Обновление списка
            ...
            if ($related) {
                ...
                foreach ($related as $item) { 
                    ...
                    $out.='<li class="relprodline" data-value="'.$product['product_id'].'">'.$product['name'].'</li>'."\n";
                }
    ...
    public function delRelated() {
        ...
        $this->load->model('extension/news');
        $this->model_extension_news->delRelatedProduct($news_id, $product_id);            
        // 
       // Обновление списка
        $out='';
        $related = $this->model_extension_news->getRelated($this->request->get['news_id']);
