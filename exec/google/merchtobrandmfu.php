...
$cattwo = 88;
        ...
        $breadcr = "Главная > Офисная техника > Лазерные МФУ";
...
foreach($prods as $pr){
    ...
    $prodarray = null;
    $prodarray[] = null;
    $breadcr = "Главная > Офисная техника > Струйные МФУ"; 
$sql = "SELECT * FROM `oc_product_to_category` WHERE `category_id` = $cattwo";
$result = $dbcnx->query($sql);
foreach($result as $ress){
$prod[] = $ress['product_id'];
...
$file=fopen("/var/www/prote/data/www/prote.ua/exec/google/merchbrandmfu.xml", "w+");
fputs($file,$output);
fclose($file);


header("Location: https://prote.ua/exec/google/merchbrandmfu.xml");
exit();
