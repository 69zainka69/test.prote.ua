...
$(document).ready(function(){
	...
	//$('#load_cart').load('index.php?route=module/popupcart'+lang, function() {
	$('#load_cart').load(lang2+'popupcart/', function() {
		...
		if($('input[name=\'click_on_cart\']').val() == 1) {
			...
			$('#cart').bind('click', function (e) { 
				...
				//$('#popupcart_extended').load('index.php?route=module/popupcart'+lang+' #popupcart_extended > *', function() {
...
var cart = {
	'add': function(product_id, quantity, flag,position) {
		language = $('html').attr('lang');
		lang='';
		if(language=='uk' || language=='uk-UA'){lang='ua/';}
		...
		//index.php?route=checkout/cart/add&product_id=39025&quantity=1

		$.ajax({
			//url: 'index.php?route=checkout/cart/add'+lang,
			url: lang2+'cartadd/',
			type: 'post',
			data: data,
			dataType: 'json',
			success: function(json) {
				...
				if (json['success']) {
					...
					//$('#cart').load('index.php?route=common/cart/info'+lang+' #cart > *');
					$('#cart').load(lang2+'cartinfo/'+' #cart > *');
					...
					addcart_gtag(product_id,position,quantity);
				}	
			},
	...
	'remove': function(key) {
		$.ajax({
			url: 'index.php?route=checkout/cart/remove'+lang,
			type: 'post',
			data: 'key=' + key,
			dataType: 'json',
			beforeSend: function() {
			...
			success: function(json) {
				...
				if (getURLVar('route') == 'checkout/cart' || getURLVar('route') == 'checkout/checkout') {
					location = 'index.php?route=checkout/cart'+lang
				} else {
					$('#cart > ul').load('index.php?route=common/cart/info'+lang+' ul li');
				}
				if(typeof(position) == 'undefined'){position=0;}
				removecart_gtag(product_id,position,quantity);
			}
		});
	}
}
...
function updateCart(product_id, key	, flag) {
	...
	$.ajax({
		...
		url: 'index.php?route=checkout/cart/edit&product_id='+product_id+lang,
		//dataType: 'html',
		dataType: 'json',
		beforeSend: function() {
			$('.qerror').remove();
		...
		success: function(data) {
			...
			//$('#cart').load('index.php?route=common/cart/info'+lang+' #cart > *', function() {
			/*$('#cart').load(lang2+'cartinfo'+' #cart > *', function() {
...
function add_class() {
	...
	$('#button-cart').addClass(p_id).attr('onclick', 'cart.add(\''+p_id+'\', '+p_q+');return false;');
