...
var Transfer = (function ($) {
    ...
    function transfer(settings) {
        ...
        $(container).html(generateTransfer(inputId, currentTimeStr));
        ...
        $(transferId).find(tabItemName).on("click", function () {
            ...
            if (!$(this).is(".tab-active")) {
                ...
                // 清空右侧数量
                $(transferId).find(selectedTotalNum).text("Всего 0 Items");
                // 未选中项
        ...
        $(listSearchId).on("keyup", function () {
            ...
            for (var j = 0; j < $(transferId).find(transferDoubleListLi).length; j++) {
                ...
                */
                xxx = $(transferId).find(transferDoubleListLi).eq(j).text().toLowerCase();
                if (!$(transferId).find(checkboxItem).eq(j).is(':checked')
                    && xxx.indexOf($(listSearchId).val().toLowerCase()) >= 0){
        ...
        $(groupListSearchId).on("keyup", function () {
            ...
            for (var j = 0; j < $(transferId).find(transferDoubleGroupListLiUlLi).length; j++) {
                ...
                */
                 xxx =$(transferId).find(transferDoubleGroupListLiUlLi).eq(j).text().toLowerCase();
                if (!$(transferId).find(groupCheckboxItem).eq(j).is(':checked')
                    &&  xxx.indexOf($(groupListSearchId).val().toLowerCase()) >= 0
                        ) {
                    // 分组 li 改为显示
        ...
        $(selectedListSearchId).keyup(function () {
            ...
            for (var i = 0; i < $(transferId).find(transferDoubleSelectedListLi).length; i++) {
                ...
                */
                xxx = $(transferId).find(transferDoubleSelectedListLi).eq(i).text().toLowerCase()
                if (xxx.indexOf($(selectedListSearchId).val().toLowerCase()) >= 0) {
    ...
    function generateTransfer(inputId, currentTimeStr) {
        var htmlStr =
            ...
            + '<div class="tab-item-name tab-item-name-' + currentTimeStr + ' tab-active">Группы</div>'
            + '<div class="tab-item-name tab-item-name-' + currentTimeStr + '">Товары</div>'
            + '</div>'
            ...
            + '<input class="transfer-double-list-search-input" type="text" id="groupListSearch_' + currentTimeStr + '" placeholder="Поиск" value="" />'
            + '</div>'
            + '</div>'
            ...
            + '<div class="transfer-double-list-header">'
            + '<div class="transfer-double-list-search">'
            + '<input class="transfer-double-list-search-input" type="text" id="listSearch_' + currentTimeStr + '" placeholder="Поиск" value="" />'
            + '</div>'
            + '</div>'
            + '<div class="transfer-double-list-content">'
            + '<div class="transfer-double-list-main">'
            ...
            + '<div class="param-item">Выбрано</div>'
            + '</div>'
            ...
            + '<input class="transfer-double-selected-list-search-input" type="text" id="selectedListSearch_' + currentTimeStr + '" placeholder="Поиск" value="" />'
            + '</div>'
            + '</div>'
