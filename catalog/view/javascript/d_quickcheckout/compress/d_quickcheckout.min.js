...
qc.Confirm = qc.Model.extend({
    ...
    update: function() {
        ...
        $('#qc_confirm_order').append(`
            <div class="qc_confirm_order_preloader">
                <img class="icon" src="image/catalog/d_quickcheckout/svg-loaders/puff.svg" />
            </div>
        `);
...
qc.ConfirmView = qc.View.extend({
    ...
    confirm: function(e) {
        ...
        if (parseFloat(products_cart_total) < 100 &&
            $('#shipping_method_form select[name=shipping_method]').val() !== 'pickup.pickup'
        ) {
            $('#qc_confirm_order .order-sum-tooltip').addClass('active');
            valid = false;
        }

        tel = $('#payment_address_telephone').val();
        $('#payment_address_telephone-error').remove();
        if(tel==''){
            valid = false;
            $("#payment_address_telephone_input").after('<div id="payment_address_telephone-error" class="text-danger">Это поле необходимо заполнить.</div>');
            preloaderStop();
