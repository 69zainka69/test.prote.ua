	...
	public function addBanner($data) {
		...
		if (isset($data['banner_image'])) {
			foreach ($data['banner_image'] as $banner_image) {
				//if(substr($banner_image['link'], 0,3)=='/ua'){
				if($banner_image['lang']=='ua'){
					$lang_id = 2;
				} else {
					$lang_id = 1;
				}

				$this->db->query("INSERT INTO " . DB_PREFIX . "banner_image SET banner_id = '" . (int)$banner_id . "', link = '" .  $this->db->escape($banner_image['link']) . "', language_id = '" . (int)$lang_id . "', image = '" .  $this->db->escape($banner_image['image']) . "', sort_order = '" . (int)$banner_image['sort_order'] . "', active = '" . (int)$banner_image['status'] . "'");
