...
class ControllerDQuickcheckoutConfirm extends Controller {
	...
	public function index($config){
        ...
        $data['text_info_confirm'] = $this->language->get('text_info_confirm');
        $data['text_info_min_order_sum'] = $this->language->get('text_info_min_order_sum');
        ...
        $json['no_call'] = $this->session->data['no_call'];
        ...
        //vdump($this->session->data);

        return $this->load->view($template, $data);
	}

	public function updateField(){
        $json['confirm'] = $this->session->data['confirm'] = array_merge(
            $this->session->data['confirm'],
            $this->request->post['confirm']
        );
        $this->session->data['comment'] = $this->session->data['confirm']['comment'];
        $this->session->data['no_call'] = $this->session->data['confirm']['no_call'];
        ...
        $json['post'] = $this->request->post['confirm'];
        $json['session'] = $this->session->data['confirm'];
    ...
    public function updateOrder(){
        ...
        if ($this->cart->hasShipping()) {
            ...
            if (isset($this->session->data['shipping_method']['title'])) {
                ...
                if ($this->session->data['shipping_method']['code']=='flat.flat') {
                   if (isset($this->session->data['shipping_method']['flat_address']))
                       $order_data['shipping_method'] .= ': ' .$this->session->data['shipping_method']['flat_address'];
                   else
                       $order_data['shipping_method'] .= ': пользователь не указал адрес!';
                }

                // Добавить адрес доставки укрпочтой
                if ($this->session->data['shipping_method']['code']=='ukrpost.ukrpost') {
                   if (isset($this->session->data['shipping_method']['ukrpost_address'])){
                       $order_data['shipping_method'] .= ': ' .$this->session->data['shipping_method']['ukrpost_address'];
                       $order_data['shipping_address'] = $this->session->data['shipping_method']['ukrpost_address'];
                   } else {
                       $order_data['shipping_method'] .= ': пользователь не указал адрес!';
                       $order_data['shipping_address'] = ': пользователь не указал адрес!';
                   }
                }

                if ($this->session->data['shipping_method']['code']=='free.free') {
                    if (isset($this->session->data['shipping_method']['free_address'])){
                       $order_data['shipping_method'] .= ': ' .$this->session->data['shipping_method']['free_address'];
                       $order_data['shipping_address'] .= ': ' .$this->session->data['shipping_method']['free_address'];
                    } else{
                       $order_data['shipping_method'] .= ': пользователь не указал адрес!';
                       $order_data['shipping_address'] .= ': пользователь не указал адрес!';
                    }
                ...
                // Добавить отделение Новой почты
                if ($this->session->data['shipping_method']['code']=='novaposhta.novaposhta' ) {
                   $order_data['shipping_method'] .=  ': ' .$this->session->data['shipping_method']['npfullpath'];
                }

                  // Добавить отделение justin
                  if ($this->session->data['shipping_method']['code']=='justin.justin' ) {
                    $order_data['shipping_method'] .=  ': ' .$this->session->data['shipping_method']['justfullpath'];
                 }
                   // Добавить отделение meest
                   if ($this->session->data['shipping_method']['code']=='meest.meest' ) {
                    $order_data['shipping_method'] .=  ': ' .$this->session->data['shipping_method']['meestfullpath'];
                 }
                // Добавить отделение Интайм
                if ($this->session->data['shipping_method']['code']=='intime.intime' ) {
                   $order_data['shipping_method'] .=  ': ' .$this->session->data['shipping_method']['infullpath'];
                }
        ...
        $order_data['no_call'] = $this->session->data['no_call'];
