...
<style>
  ...
  <script>
    $(document).ready(function(){  
      ...
      function checkInput() {
        let form = $("#register_modal_ajax");
        let typePearson = $('#register_modal_ajax input[name=\'customer_group_id\']:checked').val();
        if (typePearson == 1) {
            individualPearsonInputs.forEach(function(item) {
              item = `#register_modal_ajax ${item}`;
                if ($(item).val() != '') {
    ...
    individualPearsonInputs.forEach(function(selector) {
      validateField('.login-modal ' + selector);
    });
    ...
    $('#register_modal_ajax .register-form__type-pearson input').on('change', function() {
      let type = $('#register_modal_ajax input[name=customer_group_id]:checked').val(); 
      if (type == 4) {
        $('#register_modal_ajax .register-form__legal').addClass('register-form__legal--show'); 
      } else {
        $('#register_modal_ajax .register-form__legal').removeClass('register-form__legal--show');
      }

 });

 $("#register_modal_ajax input").on('input', function () {
  let item = $(this),
    value = item.val();
    if(value != '') {
      $(this).removeClass('register-form__input--invalid');
  ...
  $("#register_modal_ajax").submit(function(e) {
    e.preventDefault();
    id=$(this).attr('id');
    console.log($(this).serialize());
    checkInput();
    let sizeEmpty = $("#register_modal_ajax").find('.register-form__input--invalid').length;
    if(sizeEmpty === 0){
      $.ajax({
  ...
  <div id="register"><?php //echo $column_left; ?>
    <div id="content"><?php //echo $content_top; ?>
      ...
      <form action="<?php echo $action; ?>" id="register_modal_ajax" method="post" enctype="multipart/form-data" class="form-horizontal register-form">
	...
	$.ajax({
		...
		success:function(json) {
			...
			if (json['zone'] && json['zone'] != '') {
				for (i = 0; i < json['zone'].length; i++) {S
