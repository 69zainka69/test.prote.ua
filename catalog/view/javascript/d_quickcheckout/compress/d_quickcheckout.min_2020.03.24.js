...
qc.Confirm = qc.Model.extend({
    ...
    update: function() {
        //console.log('update');
        tel = $('#payment_address_telephone').val();
        if(tel==''){
            alert('Введите номер телефона');
            //$("#payment_address_telephone_input").addClass("has-error");
            //$("#payment_address_telephone_input").after('<div id="payment_address_telephone-error" class="text-danger">Это поле необходимо заполнить</div>');
            return false;
        }
        var that = this;
        var lang='';
...
qc.ConfirmView = qc.View.extend({
    ...
    confirm: function() {
        ...
        // На всякий случай, записываем в сессии значения пользовательских полей
        ...
        if (ukrpost_address || flat_address || free_address) {
            // Записываем в сессию значения пользовательских полей - их там может не быть!
            // 
            var userdata={
            ...
            console.log('confirm - передали?',userdata);
