<?php if(!$ajax){ ?>
  ...
  .logins .error {font-size:12px;line-height:13px;padding-bottom:3px;}
    ...
    <div class="login-form login-form--hide" class="row">
      ...
      <div class="login-form__content login-form__content--active">
          <form action="<?php echo $action; ?>&json=1" method="post" id="login_modal_<?php if($ajax){ ?>ajax<?php }else{ ?>noajax<?php } ?>">
            <div>
       ...
       <div class="login-form__content">
         <?php echo $register_modal_html; ?>
        </div>
    </div> 

    <script> 
    $(function() {
      let inputs = ['#login_modal_ajax .login-form__input[type="email"]', '#login_modal_ajax .login-form__input[type="password"]'];
    ...
    $("#login_modal_ajax input").on('input', function() {
        let item = $(this),
            value = item.val();
        if (value != '') {
            $(this).removeClass('login-form__input--invalid');
  ...
  function validateField(element) {
    ...
    $(".login-modal #login_modal_ajax, #login_noajax").submit(function(e) {
        e.preventDefault();
        checkInput();
