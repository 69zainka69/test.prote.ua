...
class ControllerModuleDQuickcheckout extends Controller {
    ...
    public function index() {
        ...
        $this->document->addScript('catalog/view/javascript/jquery-migrate-1.4.1.js');
        $this->document->addScript('catalog/view/javascript/d_quickcheckout/library/jquery-validate/jquery.validate.min.js');
        $this->document->addScript('catalog/view/javascript/d_quickcheckout/library/jquery-validate/localization/messages_ru.js', 'footer');
        // !        
        //$this->document->addScript('catalog/view/javascript/d_quickcheckout/library/jquery-maskedinput/jquery.maskedinput.min.js');
        // $this->document->addScript('https://cdnjs.cloudflare.com/ajax/libs/jquery.maskedinput/1.3.1/jquery.maskedinput.min.js', 'footer');
        //                 
        ...
        if(!$this->setting['general']['compress']){
            ...
            $this->document->addScript('catalog/view/javascript/d_quickcheckout/compress/d_quickcheckout.min.js?v9');
        }
    ...
    public function initialize(){
        ...
        if($this->customer->isLogged()){
            ...
            $this->session->data['accountt'] = 'logged22';
        }else{
        ...
        if($this->setting['general']['clear_session']){
            ...
            $this->session->data['confirm'] = array(
                ...
                'no_call' =>  $this->setSessionValue('no_call','confirm', $data, $account, false),
        ...
        $this->session->data['no_call'] = (!empty($this->session->data['no_call'])) ? $this->session->data['no_call'] : $data['account'][$account]['confirm']['fields']['no_call']['value'];
        $this->session->data['confirm'] = array(
            'comment' => $this->session->data['comment'],
            'no_call' => $this->session->data['no_call'],
