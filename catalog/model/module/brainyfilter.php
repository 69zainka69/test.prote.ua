...
final class ModelModuleBrainyFilter extends Model
{
     ...
     * @return void
     */
    //private function _parseBFilterParam($get=false)
    public function _parseBFilterParam($get=false)
    {
    ...
    private function _fillTmpTable()
    {
        ...
        $sqll = 'CREATE TEMPORARY TABLE ' . DB_PREFIX . self::RESULTS_TABLE . ' (PRIMARY KEY (`product_id`)) ' . $sql2;
        //vdump($sqll);
...
//        echo $sql;
    ...
    public function setData($data = array())
    {
        ...
        $sortFields = array(
            ...
            //"{$this->subquery->actualPrice}"   => 'p.sort_order',
        ...
        if (isset($data['sort'])) {
            ...
            if ($field !== false) {
                ...
                //$this->sortOrder[] = "(pp.quantity>0) DESC";
                //$this->sortOrder[] = "(pp.ifexist>0) DESC";
                $this->sortOrder[] = "pp.ifexist DESC";
                if($field=='pp.sort_order' || $data['sort']=='p.price'){
                    //$this->sortOrder[] = "(pp.quantity>0) DESC ,(pp.delivery_days>0) DESC";
                    //$this->sortOrder[] = "(pp.delivery_days>0) DESC";
                    $field = "{$this->subquery->actualPrice}";
    ...
    /**
     * Build list of filters for H1 and meta
     *
     *
     *
     */
    public function getheadingaddition ($f_group=false, $f_id=false, $f_group2=false, $f_id2=false, $title='',$return_mas=false) {
        ...
        if (count($conditions['filter'])) {
            ...
            foreach ($conditions['filter'] as $key=>$val) {
                $nr[] = 'fd.filter_group_id=' . $key . ' AND filter_id in (' . implode(',', $val) . ')';
            }

            $sSQL='SELECT distinct fg.filter_group_id, fd.filter_id, fg.name fgname, fd.name FROM `oc_filter_description` fd LEFT JOIN `oc_filter_group_description` fg ON  fg.filter_group_id= fd.filter_group_id AND fg.language_id=fd.language_id WHERE fg.language_id=' . (int)$this->config->get('config_language_id') . ' AND ('. implode (' OR ', $nr) . ')';

            $query = $this->db->query($sSQL);

            $fgnames=array();

            // массив с полным названием фильтров H1 типа А5(формат А5), 96 (количество страниц 96)
            $fullnames = array (3, 84, 90, 2462);
            ...
            foreach ($query->rows as $result) {
                ...
                //if(!$f_group2){
                if ($result['filter_group_id']==$f_group && $result['filter_id']==$f_id) continue;
                //}
                // ID filter_group_id=10 - производитель печатающего устройства - кастомизируем фильтр
                //
                // $res[]=($result['filter_group_id']==10 ? 'для ' : '') . /* $result['fgname'] . ' '. */ $result['name'];

                $resitems=array();
                if ($result['filter_group_id']==10) {
                    // формула вида "для -название бренда-"
                    $sfx = 'для ';

                    $pos = mb_strpos(mb_strtolower($title), mb_strtolower('для'));
                    if ($pos !== false) {
                        $sfx = ' ';
                    }
                    $resitems[]= ($result['filter_group_id']==10 && !$sxf_) ? $sfx : '';
                    $sxf_ = true;
                } elseif (in_array($result['filter_group_id'], $fullnames)) {
                    // вывод наименования для некоторых групп (без повтора)
                    if (!$fgnames[$result['fgname']]) {
                        $resitems[]= $result['fgname'];
                        $fgnames[$result['fgname']]=1;
                    }
                }    // 3, 84, 90, 2462

                // gdemon - если название фильтра (тут обрезаем последний символ) присутствуют в heading_title - пропускаем
                $pos = mb_strpos(mb_strtolower($title), mb_strtolower(substr($result['name'], 0, -1)));
                if ($pos !== false) {
                    continue;
                }

                $resitems[]= $result['name'];
        ...
        if (isset($conditions['stock_status'][0]) && count($conditions['stock_status'][0]) && in_array('7', $conditions['stock_status'][0])) {
            // Добавки по наличию товара
            if($this->config->get('config_language_id')==1){
                $res[] = 'в наличии';
                $query->rows['instok']['name'] = 'в наличии';
            } else {
                $res[] = 'в наявності';
                $query->rows['instok']['name'] = 'в наявності';
            }
        }

        $retString = implode(', ', $res);

        if($return_mas && isset($query->rows)){
            return $query->rows;
        } elseif($return_mas ){
            return array();
        } else {
     ...
     */
    public function prepareQueryForCategory($total=false)
    {
        ...
        if($total){
            ...
            $sql->select(array('p.*'))
                ...
                //->order(array_merge(array('(pp.quantity>0) DESC ,(pp.delivery_days>0) DESC'), $this->sortOrder))
                //->order($this->sortOrder)
                ->limit($this->productsLimit, $this->productsStart);
        } else {

            if($this->sortOrder[0]=='rand()'){

                $sql->select(array('p.product_id'))
                    ->from(array('p' => self::RESULTS_TABLE))
                    ->innerJoin(array('pp' => 'product'), 'p.product_id = pp.product_id')
                    ->innerJoin(array('pd' => 'product_description'), 'p.product_id = pd.product_id')
                    ->where('pd.language_id = ?', (int)$this->config->get('config_language_id').' AND p.price>0 AND (p.ifexist=2 OR p.model LIKE "U%")')
                    //->order(array_merge(array('(pp.quantity>0) DESC ,(pp.delivery_days>0) DESC'), $this->sortOrder))
                    ->order($this->sortOrder)
                    ->limit($this->productsLimit, $this->productsStart);
     ...
     * @return string SQL query string
     */
    public function prepareQueryForTotal($total=true)
    {
     ...
     * and all their values
     */
    public function getFilters($nocache=false)
    {
        if (!$nocache && isset(self::$_cache['filters'])) {
            return self::$_cache['filters'];
        }

        if ($this->subCategory) {
            $this->cache_data = $this->cache->get('bf_getFilters_sc_'.$this->subCategory.'.'.$this->config->get('config_language_id'), 'file');
        } elseif ($this->topCategory) {
            $this->cache_data = $this->cache->get('bf_getFilters_tc_'.$this->topCategory.'.'.$this->config->get('config_language_id'), 'file');
        }
        ...
        if ($this->subCategory) {
            $this->cache->set('bf_getFilters_sc_'.$this->subCategory.'.'.$this->config->get('config_language_id'), $output, 'file');
        } elseif ($this->topCategory) {
            $this->cache->set('bf_getFilters_tc_'.$this->topCategory.'.'.$this->config->get('config_language_id'), $output, 'file');
        }
