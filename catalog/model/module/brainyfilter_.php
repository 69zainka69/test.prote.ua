...
final class ModelModuleBrainyFilter extends Model 
{
    ...
    private function _calculateTotalsIn()
    {
        ...
        file_put_contents('/var/www/prote.com.ua/tosql.log', print_r($sql, 1), FILE_APPEND);
        $sql2 = new SqlStatement();
        $sql2->select(array(
                'id' => "af.group_id", 
                'val' => 'af.value', 
        ...
        file_put_contents('/var/www/prote.com.ua/tosql.log', print_r($sql2, 1), FILE_APPEND);
        if (count($this->aggregate)) {
            foreach ($this->aggregate as $type => $group) {
                $sql2->where("((af.type = '$type' AND af.group_id NOT IN (" . implode(',', array_keys($group)) . ")) OR af.type != '$type')");
            }
        }
        file_put_contents('/var/www/prote.com.ua/tosql.log', $sql2->__toString(), FILE_APPEND);
        $res = $this->db->query($sql2);
        //  file_put_contents('/var/www/prote.com.ua/tosql.log', print_r($res, 1), FILE_APPEND);
        return $res->rows;
    }
...
//        echo $sql;
     ...
     *
     */
    public function getheadingaddition ($f_group, $f_id) {
       ...
       if (count($conditions['filter'])) {
           ...
           foreach ($query->rows as $result) { 
               ...
               if ($result['filter_group_id']==10) {
                   ...
                   $resitems[]= $result['filter_group_id']==10 ? 'для ' : '';
    ...
    public function prepareQueryForCategory()
    {
        ...
        $sql->select(array('p.*'))
            ...
            ->order(array_merge(array('(pp.quantity>0) DESC'), $this->sortOrder))
...
//            ->distinct()
    ...
    public function getFilters($nocache=false)
    {
        ...
        // file_put_contents('/var/www/prote.com.ua/ssql.log', '****'.$sql->__toString(), FILE_APPEND );
