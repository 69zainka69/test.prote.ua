...
<style>
  ...
  #content-reg{flex-basis: auto;width: 100% !important;}
  ...
  <script>
    $(document).ready(function(){    
      ...
      function checkInput() {
        ...
        let typePearson = $('input[name=\'customer_group_id\']:checked').val();
        if (typePearson == 1) {
    ...
    $('.register-form__type-pearson input').on('change', function() {
      let type = $('input[name=customer_group_id]:checked', '.register-form__type-pearson').val(); 
      if (type == 4) {
        $('.register-form__legal').addClass('register-form__legal--show'); 
      } else {
        $('.register-form__legal').removeClass('register-form__legal--show');
      }
 });

 $("#register_ajax input").on('input', function () {
  let item = $(this),
    value = item.val();
    if(value != '') {
      $(this).removeClass('register-form__input--invalid');
  ...
  $("#register_ajax").submit(function(e) {
    ...
    checkInput();
    let sizeEmpty = $("#register_ajax").find('.register-form__input--invalid').length;
    if(sizeEmpty === 0){
      $.ajax({
  ...
  <div id="register"><?php //echo $column_left; ?>
    <div id="content-reg"><?php //echo $content_top; ?>

      <form action="<?php echo $action; ?>" id="register_ajax" method="post" enctype="multipart/form-data" class="form-horizontal register-form">
