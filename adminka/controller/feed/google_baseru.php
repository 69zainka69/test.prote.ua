<?php
class ControllerFeedGoogleBaseru extends Controller {
	private $error = array();

	public function index() {
		$this->load->language('feed/google_baseru');
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('feed/google_baseru', 'token=' . $this->session->data['token'], 'SSL')
		);

		$data['action'] = $this->url->link('feed/google_baseru', 'token=' . $this->session->data['token'], 'SSL');

		$data['cancel'] = $this->url->link('extension/feed', 'token=' . $this->session->data['token'], 'SSL');

		$data['token'] = $this->session->data['token'];

		$data['data_feed'] = HTTP_CATALOG . 'index.php?route=feed/google_baseru';
		...
		$this->response->setOutput($this->load->view('feed/google_baseru.tpl', $data));
	}

	protected function validate() {
		if (!$this->user->hasPermission('modify', 'feed/google_baseru')) {
	...
	public function install() {
		$this->load->model('feed/google_baseru');
	...
	public function category() {
		...
		$pagination->limit = 10;
		$pagination->url = $this->url->link('feed/google_baseru/category', 'token=' . $this->session->data['token'] . '&page={page}', 'SSL');
		...
		$this->response->setOutput($this->load->view('feed/google_baseru_category.tpl', $data));
	}
