...
class ModelCatalogFilter extends Model {
	public function addFilter($data) {
		...
		if (isset($data['filter'])) {
			foreach ($data['filter'] as $filter) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "filter SET filter_group_id = '" . (int)$filter_group_id . "', sort_order = '" . (int)$filter['sort_order'] . "', updated = now(), ext_filter_id = '" . $this->db->escape($filter['ext_filter_id']) . "'");
	...
	public function editFilter($filter_group_id, $data) {
		...
		if (isset($data['filter'])) {
			foreach ($data['filter'] as $filter) {
				if ($filter['filter_id']) {
					$this->db->query("INSERT INTO " . DB_PREFIX . "filter SET filter_id = '" . (int)$filter['filter_id'] . "', filter_group_id = '" . (int)$filter_group_id . "', sort_order = '" . (int)$filter['sort_order'] . "', updated = now(), ext_filter_id = '" . $this->db->escape($filter['ext_filter_id']) . "'");
				} else {
					$this->db->query("INSERT INTO " . DB_PREFIX . "filter SET filter_group_id = '" . (int)$filter_group_id . "', sort_order = '" . (int)$filter['sort_order'] . "', updated = now(), ext_filter_id = '" . $this->db->escape($filter['ext_filter_id']) . "'");
				}
	...
	public function getFilters($data) {
		$sql = "SELECT *, (SELECT name FROM " . DB_PREFIX . "filter_group_description fgd WHERE f.filter_group_id = fgd.filter_group_id 
		...
		/*LEFT JOIN " . DB_PREFIX . "filter_group_description fgd2 ON (f.filter_group_id = fgd2.filter_group_id) */
		WHERE 
		...
		if (!empty($data['filter_name'])) {
			...
			$sql .= " AND (fd.name LIKE '" . $this->db->escape($data['filter_name']) . "%'";
			$sql .= " OR fd.filter_group_id LIKE '" . $this->db->escape($data['filter_name']) . "%')";
		}
	...
	public function getFilterDescriptions($filter_group_id) {
		...
		foreach ($filter_query->rows as $filter) {
			...
			$filter_data[] = array(
				...
				'ext_filter_id'         => $filter['ext_filter_id'],
	...
	public function getTotalFilterGroups($data) {
            
                $sql="SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "filter_group` fg LEFT JOIN " . DB_PREFIX . "filter_group_description fgd ON (fg.filter_group_id = fgd.filter_group_id AND fgd.language_id = " . (int)$this->config->get('config_language_id') . ")";
                
                if (!empty($data['filter_group'])) {
                    $sql .= " WHERE fgd.name LIKE '" . $this->db->escape($data['filter_group']) . "%'";
                }
