...
class Cart {
	...
	public function __construct($registry) {
		...
		$this->db->query("DELETE FROM " . DB_PREFIX . "cart WHERE customer_id = '0' AND date_added < DATE_SUB(NOW(), INTERVAL 360 DAY)");
	...
	public function getProducts() {
		...
		foreach ($cart_query->rows as $cart) {
			...
			(SELECT GROUP_CONCAT(DISTINCT news_id SEPARATOR ',') FROM " . DB_PREFIX . "product_to_news pn where pn.`product_id`=p.`product_id`) as `news`
			...
			if ($product_query->num_rows && ($cart['quantity'] > 0)) {
				...
				$price_n = ($price + $option_price);
				$total_n = ($price + $option_price) * $cart['quantity'];

				if(isset($this->session->data['you_price'][$cart['cart_id']])){
					$total_n = $this->session->data['you_price'][$cart['cart_id']]*$cart['quantity'];
					$price_n = $this->session->data['you_price'][$cart['cart_id']];
				}
				/*vdump($this->session->data['you_price']);
				vdump($total_n);
				vdump($price_n);*/
				...
				$product_data[] = array(
					...
					'recurring'       => $recurring,
					'news'       => $product_query->row['news'],
                    'is_complex'      => $product_query->row['is_complex'],
				);
				//vdump($product_data);
			} else {
	...
	public function add($product_id, $quantity = 1, $option = array(), $recurring_id = 0) {
		...
		if (!$query->row['total']) {
			$sql = "INSERT " . DB_PREFIX . "cart SET customer_id = '" . (int)$this->customer->getId() . "', session_id = '" . $this->db->escape($this->session->getId()) . "', product_id = '" . (int)$product_id . "', recurring_id = '" . (int)$recurring_id . "', `option` = '" . $this->db->escape(json_encode($option)) . "', quantity = '" . (int)$quantity . "', date_added = NOW()";
