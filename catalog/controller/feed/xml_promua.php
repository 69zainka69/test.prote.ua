<?php
// Прайс-лист для загрузки на PROMUA
class ControllerFeedXmlPromua extends Controller {
    ...
    public function index() {
        ...
        $this->categories = array(
            '56'=>array(),      // Ленты для кассовых
            '51'=>array(),      // Фотобумага
            '22'=>array(),      // Чернилка
            '57'=>array(),      // Наклейки для авто
            '73'=>array(),      // Чистящие средства
            '31'=>array('34', '454')   // Лазерные (только ПАТРОН)
            );        
...
//        });
    ...
    protected function getProducts($category_id) {
            ...
            foreach ($products as $product) {
                ...
                $output .= '<item id="' . $product['model'] .'" selling_type="r">' . "\n";
                ...
                $output .= '<description><![CDATA[' . htmlspecialchars(strip_tags($product['tag'])) . ']]></description>' . "\n";
                $output .= '<price>' . sprintf('%01.2f', $product['price']) . '</price>' . "\n";
                ...
                $output .= '<url>' . str_replace('http://', 'https://', $this->url->link('product/product', 'path=' . $new_path . '&product_id=' . $product['product_id'], 'SSL')) . '</url>'."\n";
